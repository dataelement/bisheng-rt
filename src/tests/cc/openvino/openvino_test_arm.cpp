...
openvino_infer_det(cv::Mat img, std::string device_name = "CPU")
{
  ...
  std::string model_path =
      "/root/workspace/model_repo/ocr_lite_openvino/graphs/det_db_graph/1/"
      "model.xml";
  ov::Core core;
  std::shared_ptr<ov::Model> model = core.read_model(model_path);
  std::cout << "core.read_model success" << std::endl;
  // ov::preprocess::PrePostProcessor ppp =
  // ov::preprocess::PrePostProcessor(model); std::cout<<"PrePostProcessor
  // success"<<std::endl; ov::CompiledModel compiled_model =
  // core.compile_model(model, device_name,
  // ov::hint::performance_mode(ov::hint::PerformanceMode::THROUGHPUT),
  // ov::num_streams(ov::streams::NUMA), ov::inference_num_threads(200));
  ov::CompiledModel compiled_model = core.compile_model(
      model, device_name, ov::num_streams(32), ov::inference_num_threads(200),
      ov::affinity(ov::Affinity::NUMA), ov::enable_profiling(false));
...
{
  // std::string model_path =
  // "/home/public/models/openvino_2022_2/ch_ppocr_mobile_v2.0_cls_infer/model.xml";
  std::string model_path =
      "/root/workspace/model_repo/ocr_lite_openvino/graphs/cls_angle_graph/1/"
      "model.xml";
  ov::Core core;
  std::shared_ptr<ov::Model> model = core.read_model(model_path);
...
{
  // std::string model_path =
  // "/home/public/models/openvino_2022_2/ch_PP-OCRv3_rec_infer_fp16_matrix/model.xml";
  // std::string model_path =
  // "/root/workspace/model_repo/ocr_lite_openvino/graphs/rec_ch_graph/1/model.xml";
