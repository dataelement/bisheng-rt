        ...
        for name in names:
            ...
            if os.path.exists(os.path.join(pred_dir, name)):
                for line in open(os.path.join(pred_dir, name)):
                    ...
                    lines = list(map(float, lines))
                    box = np.array(lines).reshape([4, 2])
                    # pred_boxes[idx2label[label_idx]].append(np.int0(np.round(box)))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--gt_dir', default=None, type=str, required=True, help='gt txts.'
    )
    parser.add_argument(
        '--pred_dir', default=None, type=str, required=True, help='predict txts.'
    )
    ...
    if not args.multi_class:
        ...
        multi_class_metrics = main(
            args.gt_dir, args.pred_dir, args.iou, args.multi_class
        )
