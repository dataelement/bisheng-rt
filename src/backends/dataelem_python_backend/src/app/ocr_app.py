...
class OcrApp(BaseApp):
    def __init__(self, app_params, app_inputs, app_ouputs, **kwargs):
        super(OcrApp, self).__init__(app_params, app_inputs, app_ouputs,
                                     **kwargs)
        assert 'det_model_name' in app_params, \
            'Please specify det_model_name in config.pbtxt'
        self.det_model_name = app_params['det_model_name']['string_value']
        self.det_model_version = int(
            app_params['det_model_version']
            ['string_value']) if 'det_model_version' in app_params else -1
        self.det_model_inputs = ['image', 'params']
        self.det_model_outputs = ['boxes', 'box_scores']

        assert 'reg_model_name' in app_params, \
            'Please specify reg_model_name in config.pbtxt'
        self.reg_model_name = app_params['reg_model_name']['string_value']
        self.reg_model_version = int(
            app_params['reg_model_version']
            ['string_value']) if 'reg_model_version' in app_params else -1
        self.reg_model_inputs = ['image', 'image_shape', 'params']
        self.reg_model_outputs = ['while/Exit_1']
    ...
    def intermediate(self,
        ...
        Returns:
            context (dict): OcrApp global information
    ...
    async def infer(self, context, inputs):
        ...
        Returns:
            context (dict): OcrApp global information
            outputs (list[np.array]): [ocr_results], outputs of app
        """
        ...
        # STAGE 1 det, only support one image now
        start_time = time.time()
        det_params = np.array([json.dumps(context['params']).encode('utf-8')
                               ]).astype(np.object_)
        det_inputs = [image.astype(np.float32), det_params]
        det_outputs = self.alg_infer(det_inputs, self.det_model_name,
                                     self.det_model_version,
                                     self.det_model_inputs,
                                     self.det_model_outputs)
        ocr_results['bboxes'] = det_outputs[0].tolist()
        ocr_results['bbox_scores'] = det_outputs[1].tolist()
        timer_result['det_time'] = time.time() - start_time

        # STAGE 2 intermediate: refine box, crop image, split long sentence
        start_time = time.time()
        intermediate_inputs = [image, det_outputs[0]]
        crop_images, groups = self.intermediate(intermediate_inputs,
                                                refine_boxes, process_horizon,
                                                split_long_sentence)
        timer_result['intermediate_time'] = time.time() - start_time

        # STAGE 3 recog
        if len(crop_images):
            start_time = time.time()
            crop_images, shapes = convert_to_image_tensor(
                crop_images)  # convert to tensor (num, 32, max_width, channel)
            reg_params = np.array([
                json.dumps(context['params']).encode('utf-8')
            ]).astype(np.object_)
            reg_inputs = [crop_images.astype(np.float32), shapes, reg_params]
            reg_outputs = self.alg_infer(reg_inputs, self.reg_model_name,
                                         self.reg_model_version,
                                         self.reg_model_inputs,
                                         self.reg_model_outputs)
            reg_texts = list(map(lambda x: x.decode(), reg_outputs[0]))
            if split_long_sentence:
                reg_texts = merge_texts(reg_texts, groups, delimitor)
            ocr_results['texts'] = reg_texts
        else:
            ocr_results['texts'] = []
        timer_result['reg_time'] = time.time() - start_time

        output = dict()
        output['width'] = image.shape[1]
        output['height'] = image.shape[0]
        output['contents'] = ocr_results
        output['time'] = timer_result
