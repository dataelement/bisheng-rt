...
namespace lanms_adaptor {
	 ...
	 */
	std::vector<std::vector<float>> merge_quadrangle_standard(
			py::array_t<float, py::array::c_style | py::array::forcecast> quad_n9,
			float iou_threshold) {
		auto pbuf = quad_n9.request();
		if (pbuf.ndim != 2 || pbuf.shape[1] != 11)
			throw std::runtime_error("quadrangles must have a shape of (n, 11)");
		auto n = pbuf.shape[0];
		auto ptr = static_cast<float *>(pbuf.ptr);
		return polys2floats(lanms::merge_quadrangle_standard(ptr, n, iou_threshold));
	}
...
PYBIND11_PLUGIN(adaptor) {
	...
	m.def("merge_quadrangle_standard", &lanms_adaptor::merge_quadrangle_standard,
			"merge quadrangels");
