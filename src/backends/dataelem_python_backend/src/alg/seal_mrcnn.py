...
from . import lanms
from .alg import ALGORITHM_REGISTRY, AlgorithmWithGraph
...
def compute_curve_text_angle(box, box_cos, box_sin):
    """
    compute angle of curve box
    """
    cos_value = box_cos
    sin_value = box_sin
    cos_value_norm = 2 * cos_value - 1
    ...
    box = np.append(box, angle)
    return box


def polygon_from_points_any_shape(points):
    """
    Returns a Polygon object to use with the Polygon2 class from a list of
...
def isRotate(bbox):
    """
    bbox is rotate if bbox.area / min_rect_bbox.area > 0.8 else curve
    """
    plg_bbox = polygon_from_points_any_shape(bbox)
    bbox_area = plg_bbox.area()
    rect = cv2.minAreaRect(np.array(bbox.reshape(-1, 2), int))
    rect_box = cv2.boxPoints(rect)
    rect_box = rect_box.reshape(-1)
    plg_box = polygon_from_points_any_shape(rect_box)
    box_area = plg_box.area()
    return abs(bbox_area - box_area) < 0.2 * box_area, rect_box


class Mrcnn(AlgorithmWithGraph):
    def __init__(self, alg_params, alg_inputs, alg_ouputs, **kwargs):
        super(Mrcnn, self).__init__(alg_params, alg_inputs, alg_ouputs,
                                    **kwargs)
...
class MrcnnSealDetect(Mrcnn):
    def __init__(self, alg_params, alg_inputs, alg_ouputs, **kwargs):
        super(MrcnnSealDetect, self).__init__(alg_params, alg_inputs,
                                              alg_ouputs, **kwargs)
    ...
    def postprocess(self, context, inputs):
        ...
        boxes_len = [len(box) for box in boxes]
        max_len = max(boxes_len) if len(boxes_len) else 0
        boxes_pad = np.ones((len(boxes_len), max_len)) * -100

        for index in range(len(boxes_len)):
            boxes_pad[index, :boxes_len[index]] = boxes[index]

        post_outputs = [
            np.asarray(boxes_pad),
            np.asarray(labels),
            np.asarray(boxes_cos),
            np.asarray(boxes_sin)
        ]
        return context, post_outputs


@ALGORITHM_REGISTRY.register_module()
class MrcnnCurveTextDetect(Mrcnn):
    def __init__(self, alg_params, alg_inputs, alg_ouputs, **kwargs):
        super(MrcnnCurveTextDetect, self).__init__(alg_params, alg_inputs,
                                                   alg_ouputs, **kwargs)
    ...
    def postprocess(self, context, inputs):
        ...
        for index, box in enumerate(boxes):
            flag, rect_box = isRotate(box)
            if flag:
                # rotate box
                boxes[index] = start_point_boxes([rect_box],
                                                 [boxes_cos[index]],
                                                 [boxes_sin[index]])[0]
            else:
                # curve box
                boxes[index] = compute_curve_text_angle(
                    box, boxes_cos[index], boxes_sin[index])
        ...
        post_outputs = [np.asarray(boxes_pad)]
        return context, post_outputs


@ALGORITHM_REGISTRY.register_module()
class MrcnnQuadTextDetect(Mrcnn):
    def __init__(self, alg_params, alg_inputs, alg_ouputs, **kwargs):
        super(MrcnnQuadTextDetect, self).__init__(alg_params, alg_inputs,
                                                  alg_ouputs, **kwargs)
