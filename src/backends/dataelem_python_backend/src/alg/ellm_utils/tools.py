...
from .doc_match_label import match_label_v1
...
def compare_version(version, pair_version):
    ...
    for version_code, pair_version_code in zip(version_list,
                                               pair_version_list):
...
class DataConverter(object):
    ...
    def process_image_tag(self, line, task_type='ext'):
        ...
        assert len(text) == len(
            bbox), 'len of text is not equal to len of bbox'
        items['text'] = text
        items['bbox'] = bbox
...
class DataConverterV2(object):
    ...
    def process_image_tag(self, line, task_type='ext'):
        ...
        ocr_bboxes = []
        ocr_texts = []
        ocr_file = os.path.join(self.ocr_results,
                                os.path.splitext(img_file)[0] + '.json')
        assert os.path.exists(ocr_file), f'{ocr_file} does not exists.'
        ...
        for bbox, text in zip(ocr_bboxes, ocr_texts):
            ...
            x1, y1, x2, y2 = min(bbox[:, 0]), min(bbox[:, 1]), max(
                bbox[:, 0]), max(bbox[:, 1])
            # for model layout, only support rectangle
            layout.append(([x1, y1, x2, y2], text))
        parsed_doc = doc_parser.parse({
            'doc': img_path,
            'layout': layout
        },
                                      do_ocr=False)
        ...
        if task_type == 'ext':
            ...
            for e in result_list:
                ...
                gt_bbox = e['origin_bbox']
                gt_text = e['origin_text']
                offsets = match_label_v1(gt_bbox, gt_text, ocr_bboxes,
                                         ocr_texts)
    ...
    def convert_ext_examples(self, raw_examples, is_train=True):
        ...
        # Entity label set: ["时间", "地点", ... ]
        entity_label_set = []
        # Entity scene label set: {'fapiao': ["时间", "地点", ... ], 'idcard': []}
        entity_scene_label_set = defaultdict(list)
        # Entity name set: ["2月8日上午", "北京", ... ]
        entity_name_set = []
        # Predicate set: ["歌手", "所属专辑", ... ]
        predicate_set = []
        ...
        # List[List[str]]
        # List of entity scene for each example
        entity_scene_list = []
        # List of entity prompt for each example
        entity_prompt_list = []
        # List of relation prompt for each example
        ...
        if self.anno_type == 'text':
            images, bbox_list, image_files = None, None, None
        else:
            images, bbox_list, image_files = [], [], []

        logger.info('Converting annotation data...')
        with tqdm(total=len(raw_examples)) as pbar:
            for line in raw_examples:
                scene = line['data'].get('scene', 'doc')

                if self.anno_type == 'text':
                    items = self.process_text_tag(line, task_type='ext')
                    image, bbox, image_file = None, None, None
                elif self.anno_type == 'image':
                    items = self.process_image_tag(line, task_type='ext')
                    if items is None:
                        continue
                    image, bbox, image_file = items['image'], items[
                        'bbox'], items['image_file']
                    images.append(image)
                    bbox_list.append(bbox)
                ...
                for entity in entities:
                    ...
                    if entity_label not in entity_label_set and entity_label != '观点词':
                        entity_label_set.append(entity_label)
                    # 统计每个场景的prompt key
                    if entity_label not in entity_scene_label_set[
                            scene] and entity_label != '观点词':
                        entity_scene_label_set[scene].append(entity_label)
                    if entity_name not in entity_name_set:
                        entity_name_set.append(entity_name)
                    entity_prompt.append(entity_label)
                ...
                for relation in relations:
                    ...
                    if prompt not in relation_example_map.keys():
                        ...
                        if self.anno_type == 'image':
                            # relation_example_map[prompt]["image"] = image
                            relation_example_map[prompt]['image'] = image_file
                            relation_example_map[prompt]['bbox'] = bbox
                    else:
                        relation_example_map[prompt]['result_list'].append(
                            result)
        ...
        positive_examples, negative_examples = self.add_entity_negative_example(
            entity_examples, texts, entity_prompt_list, entity_label_set,
            entity_scene_label_set, entity_scene_list, images, bbox_list,
            image_files)
        if len(positive_examples) == 0:
            all_entity_examples = []
        else:
        ...
        if len(predicate_set) != 0:
            ...
            if is_train:
                ...
                relation_examples = self.add_full_negative_example(
                    relation_examples, texts, relation_prompt_list,
                    predicate_set, subject_golden_list, images, bbox_list,
                    image_files)
                all_relation_examples = [
                    r for relation_example in relation_examples
                    for r in relation_example
        ...
        if images and bbox_list:
            for image, image_file in zip(images, image_files):
                all_images[image_file] = image
        return all_entity_examples + all_relation_examples + entity_cls_examples, all_images
                ...
                for subject in subject_golden_list[i]:
                    for predicate in predicate_set:
                        ...
                        if prompt not in relation_prompt:
                            ...
                            if images and bbox_list:
                                # negative_result["image"] = images[i]
                                negative_result['image'] = image_files[i]
                                negative_result['bbox'] = bbox_list[i]
                            negative_sample.append(negative_result)
                examples[i].extend(negative_sample)
                pbar.update(1)
    ...
    def add_entity_negative_example(self,
        ...
        print('scene_label_set:', scene_label_set)
        with tqdm(total=len(prompts)) as pbar:
            for i, prompt in enumerate(prompts):
                scene = scenes[i]
                label_set_each_scene = scene_label_set[scene]
                redundants = list(set(label_set_each_scene) ^ set(prompt))
                # redundants = list(set(label_set) ^ set(prompt))
                redundants.sort()
                ...
                for idx in idxs:
                    ...
                    if images and bbox_list:
                        ...
                        negative_result['image'] = image_files[i]
                        negative_result['bbox'] = bbox_list[i]
                        negative_result['image_file'] = image_files[i]
                    negative_examples.append(negative_result)
                positive_examples.extend(examples[i])
                pbar.update(1)
        return positive_examples, negative_examples
    ...
    def add_relation_negative_example(self,
        ...
        for idx in idxs:
            ...
            if image and bbox:
                # negative_result["image"] = image
                negative_result['image'] = image_file
                negative_result['bbox'] = bbox
            added_example.append(negative_result)
