...
int EvaluateTag(
    const string& document, const string& document_root, int idx,
    const Value* context, TagOperator tag, const string& tag_name,
    const string& tag_arg, bool is_triple, stringstream* out);

void
EscapeHtml(const string& in, stringstream* out)
{
...
// this from being a simple split() is that we allow path components to be
// quoted, e.g.: "foo".bar, and any '.' characters inside those quoted sections
// aren't considered to be delimiters. This is to allow Json keys that contain
// periods.
void
FindJsonPathComponents(const string& path, vector<string>* components)
{
  bool in_quote = false;
  bool escape_this_char = false;
  int start = 0;
  for (int i = start; i < (int)path.size(); ++i) {
    if (path[i] == '"' && !escape_this_char)
      in_quote = !in_quote;
    ...
    if (path[start] == '"' && path[(path.size() - 1) - start] == '"') {
      if (path.size() - start > 3) {
        components->push_back(
            path.substr(start + 1, path.size() - (start + 2)));
      }
    } else {
...
// 'resolved'. If the entity does not exist (i.e. the path is invalid),
// 'resolved' will be set to NULL.
void
ResolveJsonContext(
    const string& path, const Value& parent_context, const Value** resolved)
{
  if (path == ".") {
...
int
FindNextTag(
    const string& document, int idx, TagOperator* tag_op, string* tag_name,
    string* tag_arg, bool* is_triple, stringstream* out)
{
  *tag_op = NONE;
  int document_size = document.size();
  while (idx < document_size) {
    if (document[idx] == '{' && idx < (document_size - 3) &&
        document[idx + 1] == '{') {
      if (document[idx + 2] == '{') {
        idx += 3;
        *is_triple = true;
      ...
      while (idx < document_size) {
        if (document[idx] != '}') {
          ...
          if (!*is_triple && idx < document_size - 1 &&
              document[idx + 1] == '}') {
            ++idx;
            break;
          } else if (
              *is_triple && idx < document_size - 2 &&
...
// evaluating the tag in 'parent_context'. False or non-existant values cause
// the entire section to be skipped. True values cause the section to be
// evaluated as though it were a normal section, but with the parent context
// being the root context for that section.
//
// If 'is_negation' is true, the behaviour is the opposite of the above: false
// values cause the section to be normally evaluated etc.
int
EvaluateSection(
    const string& document, const string& document_root, int idx,
...
{
  // Precondition: idx is the immediate next character after an opening {{
  // #tag_name }}
  const Value* context;
  ResolveJsonContext(tag_name, *parent_context, &context);

  // If we a) cannot resolve the context from the tag name or b) the context
  // evaluates to false, we should skip the contents of the template until a
  // closing {{/tag_name}}.
  ...
  if (op == NEGATED_SECTION_START || op == PREDICATE_SECTION_START ||
    ...
    // the context exists and is true, skip the contents, else echo them.
  ...
  BOOST_FOREACH (const Value* v, values) {
    ...
    while (idx < document_size) {
      ...
      bool is_triple;
      idx = FindNextTag(
          document, idx, &tag_op, &next_tag_name, &tag_arg, &is_triple,
          skip_contents ? NULL : out);

      if (idx > document_size)
        return idx;
      ...
      if (!skip_contents) {
        idx = EvaluateTag(
            document, document_root, idx, v, tag_op, next_tag_name, tag_arg,
            is_triple, out);
      }
    }
  }
...
// Evaluates a SUBSTITUTION tag, by replacing its contents with the value of the
// tag's name in 'parent_context'.
int
EvaluateSubstitution(
    const string& document, const int idx, const Value* parent_context,
...
// Evaluates a 'partial' template by reading it fully from disk, then rendering
// it directly into the current output with the current context.
//
// TODO: This could obviously be more efficient (and there are lots of file
// accesses in a long list context).
void
EvaluatePartial(
    const string& tag_name, const string& document_root,
...
// Given a tag name, and its operator, evaluate the tag in the given context and
// write the output to 'out'. The heavy-lifting is delegated to specific
// Evaluate*() methods. Returns the new cursor position within 'document', or -1
// on error.
int
EvaluateTag(
    const string& document, const string& document_root, int idx,
  ...
  switch (tag) {
    ...
    case INEQUALITY:
      return EvaluateSection(
          document, document_root, idx, context, tag, tag_name, tag_arg, out);
    case SUBSTITUTION:
      return EvaluateSubstitution(
          document, idx, context, tag_name, is_triple, out);
    case COMMENT:
      return idx;  // Ignored
    case PARTIAL:
...
void
RenderTemplate(
    const string& document, const string& document_root, const Value& context,
    stringstream* out)
{
  int idx = 0;
  while (idx < (int)document.size() && idx != -1) {
    ...
    bool is_triple;
    idx = FindNextTag(
        document, idx, &tag_op, &tag_name, &tag_arg, &is_triple, out);
    idx = EvaluateTag(
        document, document_root, idx, &context, tag_op, tag_name, tag_arg,
        is_triple, out);
  }
}
