...
template <typename T>
recursive_wrapper<T>::recursive_wrapper(T const& operand) : p_(new T(operand))
{
}
...
template <typename T>
void
recursive_wrapper<T>::assign(const T& rhs)
{
  this->get() = rhs;
}
...
template <typename T>
inline void
swap(recursive_wrapper<T>& lhs, recursive_wrapper<T>& rhs) noexcept
{
  lhs.swap(rhs);
}
