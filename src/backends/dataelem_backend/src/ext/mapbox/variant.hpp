...
#include "ext/mapbox/recursive_wrapper.hpp"
...
struct variant_helper<T, Types...> {
      ...
      const std::size_t old_id, void* old_value, void* new_value)
  {
    if (old_id == sizeof...(Types)) {
      ...
      // ^^  DANGER: this should only be considered for relocatable types
      //        e.g built-in types
...
template <typename T>
struct unwrapper {
  T const& operator()(T const& obj) const { return obj; }
...
struct dispatcher<F, V> {
  ...
  VARIANT_INLINE static result_type apply_const(V const&, F)
  {
    throw std::runtime_error(
        std::string("unary dispatch: FAIL ") + typeid(V).name());
  }
...
struct binary_dispatcher_rhs<F, V, T0, T1, Types...> {
  ...
  {
    // call binary functor
    if (rhs.get_type_index() == sizeof...(Types)) {
      return f(
          unwrapper<T0>()(lhs.template get<T0>()),
...
struct binary_dispatcher_lhs<F, V, T0, T1, Types...> {
  ...
  {
    // call binary functor
    if (lhs.get_type_index() == sizeof...(Types)) {
      return f(lhs.template get<T1>(), rhs.template get<T0>());
    } else {
      return binary_dispatcher_lhs<F, V, T0, Types...>::apply_const(
          lhs, rhs, f);
    }
  }
...
class variant {
 ...
 public:
  VARIANT_INLINE variant() : type_index(sizeof...(Types) - 1)
  {
    new (&data) typename detail::select_type<0, Types...>::type();
  }

  VARIANT_INLINE variant(no_init) : type_index(detail::invalid_value) {}

  // http://isocpp.org/blog/2012/11/universal-references-in-c11-scott-meyers
  template <
      typename T,
      class = typename std::enable_if<detail::is_valid_type<
          typename std::remove_reference<T>::type, Types...>::value>::type>
  VARIANT_INLINE variant(T&& val) noexcept
      : type_index(detail::value_traits<
                   typename std::remove_reference<T>::type, Types...>::index)
  {
  ...
  VARIANT_INLINE std::size_t get_type_index() const { return type_index; }
  ...
  auto VARIANT_INLINE static binary_visit(V const& v0, V const& v1, F f)
      -> decltype(
          detail::binary_dispatcher<F, V, Types...>::apply_const(v0, v1, f))
  {
  ...
  ~variant() noexcept { helper_type::destroy(type_index, &data); }
...
auto VARIANT_INLINE static apply_visitor(F f, V const& v)
    -> decltype(V::visit(v, f))
{
  return V::visit(v, f);
}
...
// const
template <typename V, typename F>
auto VARIANT_INLINE static apply_visitor(F f, V const& v0, V const& v1)
...
// non-const
template <typename V, typename F>
auto VARIANT_INLINE static apply_visitor(F f, V& v0, V& v1)
    -> decltype(V::binary_visit(v0, v1, f))
{
  return V::binary_visit(v0, v1, f);
}
