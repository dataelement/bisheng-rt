...
Utility::ReadDict(const std::string& path)
{
...
void
Utility::GetAllFiles(const char* dir_name, std::vector<std::string>& all_inputs)
{
  if (NULL == dir_name) {
  ...
  if (!S_ISDIR(s.st_mode)) {
    ...
    while ((filename = readdir(dir)) != NULL) {
      ...
      all_inputs.push_back(
          dir_name + std::string("/") + std::string(filename->d_name));
    }
  }
}

cv::Mat
Utility::GetRotateCropImage(
    const cv::Mat& srcimage, std::vector<std::vector<int>> box)
{
  ...
  int img_crop_width = int(sqrt(
      pow(points[0][0] - points[1][0], 2) +
      pow(points[0][1] - points[1][1], 2)));
  int img_crop_height = int(sqrt(
  ...
  cv::Mat dst_img;
  cv::warpPerspective(
      img_crop, dst_img, M, cv::Size(img_crop_width, img_crop_height),
      cv::BORDER_REPLICATE);
...
std::vector<int>
Utility::argsort(const std::vector<float>& array)
{
...
std::string
Utility::basename(const std::string& filename)
{
  if (filename.empty()) {
    return "";
  }
...
bool
Utility::PathExists(const std::string& path)
{
#ifdef _WIN32
  struct _stat buffer;
...
void
Utility::CreateDir(const std::string& path)
{
#ifdef _WIN32
  _mkdir(path.c_str());
#else
...
void
Utility::print_result(const std::vector<OCRPredictResult>& ocr_result)
{
