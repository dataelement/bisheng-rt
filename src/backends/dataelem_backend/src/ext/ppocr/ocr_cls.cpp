...
void
Classifier::Run(
    std::vector<cv::Mat> img_list, std::vector<int>& cls_labels,
    std::vector<float>& cls_scores, std::vector<double>& times)
{
  ...
  for (int beg_img_no = 0; beg_img_no < img_num;
    ...
    for (int ino = beg_img_no; ino < end_img_no; ino++) {
      ...
      cv::Mat resize_img;
      this->resize_op_.Run(
          srcimg, resize_img, this->use_tensorrt_, cls_image_shape);
    ...
    std::vector<float> input(
        batch_num * cls_image_shape[0] * cls_image_shape[1] *
            cls_image_shape[2],
        0.0f);
...
void
Classifier::LoadModel(const std::string& model_dir)
{
