...
void
CRNNRecognizer::Run(
    std::vector<cv::Mat> img_list, std::vector<std::string>& rec_texts,
    std::vector<float>& rec_text_scores, std::vector<double>& times)
{
  ...
  for (int beg_img_no = 0; beg_img_no < img_num;
    ...
    // std::cout << "output d2h time: " << diff.count() * 1000 << "ms\n";
...
void
CRNNRecognizer::LoadModel(const std::string& model_dir)
{
