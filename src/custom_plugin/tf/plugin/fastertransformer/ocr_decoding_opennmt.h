...
#include "fastertransformer/ocr_open_decoder.h"
    ...
    */
    ...
    for (int step = 1; step <= seq_len_; ++step)
    {
      ...
      */
      ...
      sine_position_encoder<DataType_>(from_tensor_[0], step - 1, m, hidden_units_, decoding_params.stream);
      ...
      check_cuda_error(cublasGemmEx(decoding_params.cublas_handle,
                                    ...
                                    n, m, k,
                                    &alpha,
                                    decoding_params.embedding_table, AType_, k,
      ...
      update_logits_v2(logits_buf_, end_id_, finished_buf_, m, n, decoding_params.stream);
#else
      // update_logits(logits_buf_, decoding_params.embedding_bias, end_id_, finished_buf_, m, n, decoding_params.stream);
      update_logits_v2(logits_buf_, end_id_, finished_buf_, m, n, decoding_params.stream);
      cudaDeviceSynchronize();
      ...
      */
      ...
      if(sum == batch_size_ * beam_width_){
        break;
      }
    }
    check_cuda_error(cudaMemcpy(decoding_params.cum_log_probs, cum_log_buf_,
                                  sizeof(float)*m, cudaMemcpyDeviceToDevice));
  }
