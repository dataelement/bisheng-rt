...
{
template <typename T>
struct OcrTransformerOpFunctor<GPUDevice, T>
{
  typedef typename TFTraits<T>::DataType DataType_;
  static Status Compute(OpKernelContext *context,
      EncoderInitParamOCR<DataType_ > param,
      OcrEncoderTransformer<OcrEncoderTransformerTraits< TFTraits<T>::OpType,
      cuda::OpenMultiHeadAttentionOcr > > *encoder_transformer)
  {
...
template struct functor::OcrTransformerOpFunctor<GPUDevice, float>;
template struct functor::OcrTransformerOpFunctor<GPUDevice, Eigen::half>;
} //namespace tensorflow
#endif
