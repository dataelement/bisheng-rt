...
template <>
__global__
void add_QKV_bias(half* Q, half* K, half* V,
...
template <typename T>
__global__
void softmax_kernel(T* qk_buf_, T* qk_buf_sub_, const T* attr_mask, const int batch_size, const int head_num, const int seq_len, const T scalar)
{
    ...
    for(int i = 0; i < seq_len; ++i)
    {
      ...
      if(threadIdx.x < seq_len)
        // qk_buf_sub_[threadIdx.x + qk_offset] = (T)(qk / s_sum);
...
template <typename T>
__global__
void softmax_kernel_v2(T* qk_buf_, T* qk_buf_sub_, const T* attr_mask, const int batch_size, const int head_num, const int seq_len, const float scalar)
{
    ...
    if(threadIdx.x < seq_len)
      // qk_buf_sub_[threadIdx.x + qk_offset] = (T)(qk_tmp / s_sum);
      qk_buf_[threadIdx.x + qk_offset] = (T)(qk_tmp / s_sum);
}

template<typename T>
__global__
void transpose_ocr(T* src, T* dst, const int batch_size, const int seq_len, const int head_num, const int size_per_head)
{
  int batch_id = blockIdx.x / (head_num * seq_len);
  int seq_id = blockIdx.x % seq_len;
...
template<>
  __global__
void transpose_ocr(half* src, half* dst,
...
void OpenMultiHeadAttentionOcr<OpType_>::multiHeadAttr_nofuse_kernelLauncher(
      cudaStream_t stream,
      ...
      const DataType_ scalar)
{
    ...
    if(OpType_ == OperationType::FP32)
    {
      ...
      dim3 block(k);
      add_QKV_bias<DataType_><<<grid, block, 0, stream>>>(Q, K, V, q_buf_, k_buf_, v_buf_, batch_size, seq_len, head_num, size_per_head, word_per_block);
    }
    else
    {
      const int word_per_block = 1;
      ...
      add_QKV_bias<DataType_><<<grid, block, 0, stream>>>(Q, K, V, q_buf_, k_buf_,
    ...
    if(batch_size * head_num <= 120)
    {
      ...
      softmax_kernel_v2<DataType_><<<grid, block, 0, stream>>>(qk_buf_, qk_buf_sub_, attr_mask, batch_size, head_num, seq_len, scalar);
      // check_cuda_error(cudaMemcpy(qk_buf_, qk_buf_sub_, sizeof(DataType_)*16*512, cudaMemcpyDeviceToDevice));
    }
    else
    {
      grid.x = batch_size * head_num;
      softmax_kernel<DataType_><<<grid, block, 0, stream>>>(qk_buf_, qk_buf_sub_, attr_mask, batch_size, head_num, seq_len, scalar);
      // check_cuda_error(cudaMemcpy(qk_buf_, qk_buf_sub_, sizeof(DataType_)*16*512, cudaMemcpyDeviceToDevice));
    }
...
/* for half2 only */
  ...
  //    const int seq_per_block = 1;
      ...
      transpose_ocr<DataType_><<<grid, block, 0, stream>>>(transpose_dst_, dst,
...
template void OpenMultiHeadAttentionOcr<OperationType::FP32>::multiHeadAttr_nofuse_kernelLauncher(
      cudaStream_t stream,
...
template void OpenMultiHeadAttentionOcr<OperationType::FP16>::multiHeadAttr_nofuse_kernelLauncher(
      cudaStream_t stream,
      cublasHandle_t handle,
      half* Q,
      half* K,
