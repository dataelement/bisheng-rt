...
class OpenMultiHeadAttentionOcrTraits;

template<>
class OpenMultiHeadAttentionOcrTraits<OperationType::FP32>
{
 public:
...
template<>
class OpenMultiHeadAttentionOcrTraits<OperationType::FP16>
{
 public:
 ...
 */
template<OperationType OpType_>
class OpenMultiHeadAttentionOcr: IMultiHeadAttention<OpType_>
{
 private:
  typedef OpenMultiHeadAttentionOcrTraits<OpType_> Traits_;
 ...
 public:
  //Ctor
  OpenMultiHeadAttentionOcr(const IAllocator& allocator, int batch_size, int from_seq_len,
    ...
    head_num_(head_num), size_per_head_(size_per_head)
   {
    ...
    try
    {
      buf_ = (DataType_*) allocator_.malloc(sizeof(DataType_) * (buf_size * 7 + qk_buf_size * 2));
      query_buf_ = buf_;
      ...
      qk_buf_sub_ = qk_buf_ + qk_buf_size;
      transpose_dst_ = qk_buf_ + qk_buf_size * 2;
  ...
  ~OpenMultiHeadAttentionOcr() override
  {
