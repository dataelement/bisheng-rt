  ...
  const int step)
{
  ...
  masked_attention_kernel<DataType_><<<grid, block, shared_size, param_.stream>>>(
    query_buf_, key_cache_, value_cache_,
    context_buf_, batch_size_, head_num_, size_per_head_, step, scalar);
  ...
  int m, int n)
{
  ...
  */
  ...
  /* should pay attention to the rsqrt precision*/
  decoder_norm2_kernel<DataType_><<<grid, block, 0, param_.stream>>>(input, gamma, beta, output, norm_output, m, n);
}
