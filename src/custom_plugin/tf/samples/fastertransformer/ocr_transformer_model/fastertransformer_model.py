...
def build_resnet_vd(inputs, dtype):
...
def op_encoder(inputs,
               ...
               attention_mask,
               ffn_mask):
    transformer_op_module = tf.load_op_library('../../build/lib/libtf_ocr_encode_op.so')
    N = len(encoder_vars)
    for layer_idx in range(encoder_args.num_layer):
        val_off = layer_idx * 20
        print(encoder_vars[val_off + 0].name)
        outputs = transformer_op_module.ocr_transformer(
            inputs,
            attention_mask,
            ffn_mask,
            encoder_vars[val_off + 0],
            encoder_vars[val_off + 1],
            encoder_vars[val_off + 2],
            encoder_vars[val_off + 3],
            encoder_vars[val_off + 4],
            encoder_vars[val_off + 5],
            encoder_vars[val_off + 6],
            encoder_vars[val_off + 7],
            encoder_vars[val_off + 8],
            encoder_vars[val_off + 9],
            encoder_vars[val_off + 10],
            encoder_vars[val_off + 11],
            encoder_vars[val_off + 12],
            encoder_vars[val_off + 13],
            encoder_vars[val_off + 14],
            encoder_vars[val_off + 15],
            encoder_vars[val_off + 16],
            encoder_vars[val_off + 17],
            encoder_vars[val_off + 18],
            encoder_vars[val_off + 19],
            encoder_vars[N-2],
            encoder_vars[N-1],
            head_num=encoder_args.head_num, size_per_head=encoder_args.size_per_head,
            is_last_layer=layer_idx==encoder_args.num_layer-1)
        print(encoder_vars[N-2].name, encoder_vars[N-1].name)
        inputs = outputs
    return outputs
...
def length_normalization(alpha, length):
...
def tf_decoding(memory_tensor,
    ...
    with tf.variable_scope("Transformer", reuse=tf.AUTO_REUSE):
        ...
        _, cum_log_probs, _, _, outputs, debug_output, _, extra_vars, _, _ = tf.while_loop(
            _cond,
            _body,
            loop_vars=(
                word_ids,
        ...
        length_norm = length_normalization(0.6, finalized_tf_sequence_lengths)
        cum_log_probs = tf.reshape(cum_log_probs, tf.shape(finalized_tf_sequence_lengths))
        cum_log_scores = cum_log_probs / length_norm

        return finalized_tf_output_ids, finalized_tf_sequence_lengths, tf_output_ids, tf_parent_ids, tf_sequence_lengths, cum_log_scores, debug_output
...
def op_decoding(memory_tensor,
                ...
                decoding_args):
    extra_decode_length = 0
    decoding_op_module = tf.load_op_library('../../build/lib/libtf_ocr_decoding_op.so')
    ...
    for i in range(val_off):
        ...
        for j in range(decoding_args.decoder_args.num_layer):
            par.append(decoding_vars[i + j * val_off + 1])
        decoding_vars_in_differ_layers.append(par)

    print(decoding_vars[-2].name)
    print(decoding_vars[-1].name)
    print(decoding_vars[0].name)
    ...
    extended_memory_sequence_length = tf.reshape(extended_memory_sequence_length,
                                                 [batch_size, decoding_args.decoder_args.beam_width])
    ...
    output_ids, parent_ids, sequence_lengths, cum_log_probs = decoding_op_module.decoding(
        ...
        decoding_vars[-2], decoding_vars[-1], decoding_vars[0],
        batch_size=decoding_args.decoder_args.batch_size,
    ...
    length_norm = length_normalization(0.6, finalized_sequence_lengths)
    cum_log_probs = tf.reshape(cum_log_probs, tf.shape(finalized_sequence_lengths))
    cum_log_scores = cum_log_probs / length_norm

    return finalized_output_ids, finalized_sequence_lengths, output_ids, parent_ids, sequence_lengths, cum_log_scores
