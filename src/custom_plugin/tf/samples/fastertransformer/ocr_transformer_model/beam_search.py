...
class SequenceBeamSearch(object):
    ...
    def __init__(self, symbols_to_logits_fn, vocab_size, batch_size, beam_size, alpha,
                 max_decode_length, eos_id):
    ...
    def search(self, initial_ids, initial_cache):
        ...
        prob_matrix = finished_state[_StateKeys.ALIVE_CACHE]["prob_matrix"]
        finished_seq = finished_state[_StateKeys.FINISHED_SEQ]
        finished_scores = finished_state[_StateKeys.FINISHED_SCORES]
        finished_flags = finished_state[_StateKeys.FINISHED_FLAGS]
        debug_output = finished_state[_StateKeys.DEBUG_OUTPUT]
        ...
        return finished_seq, finished_scores, prob_matrix, debug_output
    ...
    """
        ...
        alive_cache = nest.map_structure(lambda t: _expand_to_beam_size(t, self.beam_size),
                                         initial_cache)
        ...
        # debug output
        debug_output = tf.zeros([self.batch_size, self.beam_size, self.vocab_size], tf.float32)
        ...
        state = {
            ...
            _StateKeys.MASK: _expand_to_beam_size(initial_cache["mask"], self.beam_size),
            _StateKeys.DEBUG_OUTPUT: debug_output
        }
        ...
        state_shape_invariants = {
            ...
            _StateKeys.MASK: tf.TensorShape([None, self.beam_size, self.vocab_size]),
            _StateKeys.DEBUG_OUTPUT: tf.TensorShape([None, self.beam_size, self.vocab_size])
        }
    ...
    """
        ...
        return tf.logical_and(not_at_max_decode_length,
    ...
    """
        ...
        new_seq, new_log_probs, new_cache, debug_output = self._grow_alive_seq(state)
        ...
        new_state[_StateKeys.MASK] = state[_StateKeys.MASK]
        new_state[_StateKeys.DEBUG_OUTPUT] = debug_output
        return [new_state]
    ...
    """
        ...
        flat_logits, flat_cache, debug_output = self.symbols_to_logits_fn(flat_ids, i, flat_cache)
        debug_output = tf.reshape(debug_output, [self.batch_size, self.beam_size, self.vocab_size])
        ...
        # locations whose values are not 1 will be set to -INF
        candidate_log_probs = tf.add(candidate_log_probs,
                                     (1. - tf.to_float(state[_StateKeys.MASK])) * -INF)

        # shape: [batch_size, beam_size, seq_length, vocab_size]
        new_cache[_StateKeys.PROB_MATRIX] = tf.concat(
            [new_cache[_StateKeys.PROB_MATRIX],
             tf.expand_dims(candidate_log_probs, axis=2)],
            axis=2)
        ...
        topk_seq, new_cache = _gather_beams([alive_seq, new_cache], topk_beam_indices,
                                            self.batch_size, beams_to_keep)

        # Append the most probable IDs to the topk sequences
        topk_ids = topk_indices % self.vocab_size
        topk_ids = tf.expand_dims(topk_ids, axis=2)
        topk_seq = tf.concat([topk_seq, topk_ids], axis=2)
        return topk_seq, topk_log_probs, new_cache, debug_output
    ...
    """
        ...
        top_finished_seq, top_finished_scores, top_finished_flags = (_gather_topk_beams(
            [finished_seq, finished_scores, finished_flags], finished_scores, self.batch_size,
            self.beam_size))
...
def sequence_beam_search(symbols_to_logits_fn, initial_ids, initial_cache, vocab_size, beam_size,
  ...
  """
    ...
    sbs = SequenceBeamSearch(symbols_to_logits_fn, vocab_size, batch_size, beam_size, alpha,
                             max_decode_length, eos_id)
