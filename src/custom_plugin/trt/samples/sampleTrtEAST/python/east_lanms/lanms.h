...
inline void print_poly(const Polygon& poly, int i) {
  std::cout << i << " ";
  for (unsigned k =0; k < poly.poly.size(); k++) {
    std::cout << poly.poly[k].X << " " << poly.poly[k].Y << " ";
  }
  std::cout << poly.score << "\n";
}
...
//                        print_poly(poly, i);
        ...
        if (i == n-1) {
          bpoly.score = bpoly.score / merge_num;
        }
      } else {
//						 std::cout<<bpoly.score<<' '<<merge_num<<' '<<bpoly.score / merge_num<<' ';
//						printf("\n");
//                        print_poly(poly, i);
        bpoly.score = bpoly.score / merge_num;
