...
class SampleEast: public SampleModel {

 public:
  SampleEast(SampleEastParams& params) : SampleModel(params)
    , mParams(params) {
  }
 ...
 private:
  SampleEastParams mParams; //!< The parameters for the sample.
...
bool SampleEast::predict() {
  ...
  for (unsigned int i = 0; i < names.size(); i = i + mParams.batchSize) {
    ...
    std::vector<float> im;
    std::vector<int> imgSzie;
    if (!processInput(mParams.inputTensorNames[0], binFile, shapeFile, ratio_h, ratio_w, imgSzie, im)) {
      return false;
    }
    ...
    if(cnt / mParams.batchSize >= 100 && !start_cnt) {
      start_cnt = true;
      cnt = 0;
      total = 0;
    }
    ...
    outputCorrect &= verifyOutput(mParams.outputTensorNames, ratio_h, ratio_w,
                                  mParams.dataDirs[1],
                                  names[i], outputs);
  }
...
bool SampleEast::processInput(
  ...
  const std::string& shapeFiles,
  float& ratio_hs,
  float& ratio_ws,
  std::vector<int>& imgSzie,
  std::vector<float>& im) const {
  int imageSzie = mParams.inputH * mParams.inputW * 3;
  std::ifstream f_shape(shapeFiles, std::ios::binary);
  ...
  ratio_hs = shape[3];
  ratio_ws = shape[4];
  std::ifstream f_im(binFiles, std::ios::binary);
  im.resize(size);
...
SampleEastParams initializeSampleParams(const samplesCommon::Args& args) {
  ...
  params.engineFile = "../test_data/models/east_fix_trt.trt";

  if (args.dataDirs.empty()) { //!< Use default directories if user hasn't provided directory paths
    // input images
    params.dataDirs.push_back("../test_data/ocr_east_data/im_raw_fix_nhwc");
    // result save path
    params.dataDirs.push_back("../test_data/ocr_east_data/sampleTrtEAST_fix_trt_fp32");
