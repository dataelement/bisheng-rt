/* Copyright 2019 The Authors From 4paradigm, Inc.. All Rights Reserved.
...
class fileops {
 public:
  ...
  static int files_in_dir(std::string dirent, std::vector<std::string> &files) {
    DIR *p_dir;
    struct dirent *p_dirent;

    if((p_dir = opendir((dirent).c_str())) == NULL) {
      std::cout << "check pir path:" << (dirent).c_str() << "failed" << std::endl;
      return -1;
    }
  ...
  static int files_in_txt(std::string txt_name, std::vector<std::string> &files) {
    FILE * fp = NULL;
    fp = fopen(txt_name.c_str(), "r");
    char str[1000 + 1];
    ...
    if ((dir = opendir(repo.c_str())) != NULL) {
      while ((ent = readdir(dir)) != NULL) {
        ...
        if (sub_files &&
            ...
            ent->d_name[0] != '.') {
          list_directory(std::string(repo) + "/" +
                         std::string(ent->d_name), files, dirs, sub_files, lfiles);
        }
      }
      closedir(dir);
      return 0;
  ...
  static void get_nvidia_devices(std::vector<std::string>& nv_devices, std::string& devices_str) {
