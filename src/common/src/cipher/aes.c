...
/* Includes:                                                                 */
/*****************************************************************************/
#include "triton/common/cipher/aes.h"
#include <stdint.h>
...
static void
AddRoundKey(uint8_t round, state_t* state, uint8_t* RoundKey)
{
  uint8_t i, j;
  for (i = 0; i < 4; ++i) {
    for (j = 0; j < 4; ++j) {
...
Multiply(uint8_t x, uint8_t y)
{
  return (
      ...
      ((y >> 4 & 1) *
       xtime(xtime(
           xtime(xtime(x)))))); /* this last call to xtime() can be omitted */
}
#else
#define Multiply(x, y)                         \
  (((y & 1) * x) ^ ((y >> 1 & 1) * xtime(x)) ^ \
...
static void
InvCipher(state_t* state, uint8_t* RoundKey)
{
  uint8_t round = 0;
...
void
AES_ECB_encrypt(struct AES_ctx* ctx, uint8_t* buf)
{
...
void
AES_ECB_decrypt(struct AES_ctx* ctx, uint8_t* buf)
{
...
static void
XorWithIv(uint8_t* buf, uint8_t* Iv)
{
  uint8_t i;
