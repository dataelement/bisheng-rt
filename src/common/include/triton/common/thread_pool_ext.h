...
void
parallel_run(FUNC func, ThreadPool& tp = global_thread_pool())
{
...
template <typename FUNC>
void
parallel_run_range(size_t n, FUNC func, ThreadPool& tp = global_thread_pool())
{
  size_t concurrency = tp.size();
  std::cout << "----parallel_run_range:" << concurrency << "\n";
  if (n < concurrency) {
    auto ret = tp.enqueue([&func, &n]() {
      func(0, 0, n);
      return true;
    });
    ret.get();
...
template <typename FUNC>
void
parallel_run_dynamic(size_t n, FUNC func, ThreadPool& tp = global_thread_pool())
{
