...
class StyleDict(dict):
    ...
    def get_color(self, k, d=None):
        ...
        if hasattr(color, 'startswith') and color.startswith('#'):
            color = color[1:]
            # Premailers reduces colors like #00ff00 to #0f0
            if len(color) == 3:
...
class Element(object):
    """
    Our base class for representing an html element along with a cascading
    style. The element is created along with a parent so that the StyleDict
    that we store can point to the parent's StyleDict.
    """
    def __init__(self, element, parent=None):
        self.element = element
        self.number_format = None
        parent_style = parent.style_dict if parent else None
        self.style_dict = StyleDict(style_string_to_dict(
            element.get('style', '')),
                                    parent=parent_style)
    ...
    def get_dimension(self, dimension_key):
        """
        Extracts the dimension from the style dict of the Element and returns
        it as a float.
        """
...
class Table(Element):
    """
    The concrete implementations of Elements are semantically named for the
    types of elements we are interested in. This defines a very concrete tree
    structure for html tables that we expect to deal with. I prefer this
    compared to allowing Element to have an arbitrary number of children and
    dealing with an abstract element tree.
    """
    def __init__(self, table):
        ...
        self.head = TableHead(table_head,
                              parent=self) if table_head is not None else None
        table_body = table.find('tbody')
        self.body = TableBody(table_body if table_body is not None else table,
                              parent=self)
...
class TableCell(Element):
    ...
    CELL_TYPES = {
        ...
        'TYPE_ERROR', 'TYPE_FORMULA_CACHE_STRING', 'TYPE_INTEGER'
    }
    ...
    def data_type(self):
        cell_types = self.CELL_TYPES & set(
            self.element.get('class', '').split())
        if cell_types:
            if 'TYPE_FORMULA' in cell_types:
                # Make sure TYPE_FORMULA takes precedence over the other
                # classes in the set.
                cell_type = 'TYPE_FORMULA'
            elif cell_types & {
                    'TYPE_CURRENCY', 'TYPE_INTEGER', 'TYPE_PERCENTAGE'
            }:
...
def write_rows(worksheet, elem, row, column=1):
    ...
    for table_row in elem.rows:
        for table_cell in table_row.cells:
            ...
            if colspan == 1:
                # Initially, when iterating for the first time through the
                # loop, the width of all the cells is None. As we start filling
                # in contents, the initial width of the cell (which can be
                # retrieved by: worksheet.column_dimensions[
                # get_column_letter(column)].width) is equal to the width of
...
def table_to_sheet(table, wb, index=0):
    """
...
def document_to_workbook(doc, wb=None, base_url=None):
    ...
    inline_styles_doc = Premailer(doc, base_url=base_url,
                                  remove_classes=False).transform()
    tables = get_Tables(inline_styles_doc)

    for index, table in enumerate(tables):
        table_to_sheet(table, wb, index)
...
def document_to_xl(doc, filename, base_url=None):
    """
    Takes a string representation of an html document and writes one sheet for
    every table in the document. The workbook is written out to a file called
    filename
    """
