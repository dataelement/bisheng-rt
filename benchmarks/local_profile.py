...
class QwenProfile:
    def __init__(self, model_path, device="cuda:0", seed=1024, quant_type="bf16", **kwargs) -> None:
        self.seed = seed
        ...
        if quant_type == "bf16":
            ...
            assert self.use_flash_attn is False, "FP32 profiling cannot be performed with Flash-Attention activated."
            self.model = AutoModelForCausalLM.from_pretrained(
                model_path, 
                device_map=device, 
                trust_remote_code=True, 
        ...
        self.config = GenerationConfig.from_pretrained(model_path, trust_remote_code=True)
        
    def profile(self, max_experiment_times=1, context_length_per_experiment=1, generate_length_per_experiment=2048):
        self.config.min_length = generate_length_per_experiment + context_length_per_experiment
...
class ChatGlmProfile:
    ...
    def profile(self, max_experiment_times=1, context_length_per_experiment=1, generate_length_per_experiment=2048):
        min_length = generate_length_per_experiment + context_length_per_experiment
...
if __name__ == "__main__":
    ...
    # model_profile = QwenProfile(model_path, device=device, quant_type=quant_type)

    model_path = "/home/public/llm/chatglm2-6b"
    model_profile = ChatGlmProfile(model_path, device=device, quant_type=quant_type)
    ...
    model_profile.profile(max_experiment_times=max_experiment_times, 
                          context_length_per_experiment=context_length_per_experiment, 
                          generate_length_per_experiment=generate_length_per_experiment)